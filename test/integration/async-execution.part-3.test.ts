...
`);
	...
	it("keeps configured model fallback orchestration in the background", { skip: !isAsyncAvailable() || !createSubagentExecutor ? "jiti or executor not available" : undefined }, async () => {
		mockPi.onCall({ exitCode: 1, stderr: "Provider temporarily unavailable", matchArgIncludes: "Use async model fallback" });
		mockPi.onCall({ output: "Async fallback completed", matchArgIncludes: "Use async model fallback" });
		const state = {
			baseCwd: tempDir,
			currentSessionId: null,
			asyncJobs: new Map(),
		...
		const launch = await executor.execute(
			"async-configured-model-fallback",
			{ agent: "worker", task: "Use async model fallback", async: true, acceptance: false },
			new AbortController().signal,
			undefined,
			context,
		) as AsyncExecutionResult;
		...
		const calls = [readMockPiArgs(mockPi, 0), readMockPiArgs(mockPi, 1)];

...
describe("single sync execution", { skip: !available ? "pi packages not available" : undefined }, () => {
	...
	it("expands configured fallback models into fresh sequential child launches", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		mockPi.onCall({ exitCode: 1, stderr: "Provider temporarily unavailable", matchArgIncludes: "Use model fallback" });
		mockPi.onCall({ output: "Fallback child completed", matchArgIncludes: "Use model fallback" });
		...
		const result = await executor.executePublic(
			...
			{ agent: "worker", task: "Use model fallback", async: false, context: "fresh" },
			new AbortController().signal,
			undefined,
			ctx,
		);
		...
		assert.match(result.content[0]?.text ?? "", /Fallback child completed/);
		assert.doesNotMatch(result.content[0]?.text ?? "", /artifactPaths/);
		assert.equal(mockPi.callCount(), 2, JSON.stringify(result.details));
		const calls = readAllCallArgs(true);
		assert.equal(calls[0]?.[calls[0].indexOf("--model") + 1], "openai/gpt-5-mini");
		assert.equal(calls[1]?.[calls[1].indexOf("--model") + 1], "anthropic/claude-sonnet-4-5");
		const sessions = calls.map((args) => args[args.indexOf("--session") + 1]);
		assert.notEqual(sessions[0], sessions[1]);
	});

	it("fails the outer call after configured model fallbacks are exhausted", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		mockPi.onCall({ exitCode: 1, stderr: "Provider temporarily unavailable", matchArgIncludes: "Exhaust model fallback" });
		mockPi.onCall({ exitCode: 1, stderr: "Authentication failed", matchArgIncludes: "Exhaust model fallback" });
		const executor = makeExecutor([makeAgent("worker", {
			model: "openai/gpt-5-mini",
			fallbackModels: ["anthropic/claude-sonnet-4-5"],
		})]);
		...
		const result = await executor.executePublic(
			"exhausted-model-fallback",
			{ agent: "worker", task: "Exhaust model fallback", async: false, context: "fresh", acceptance: false },
			new AbortController().signal,
			undefined,
			ctx,
		);
	...
	it("applies configured model fallbacks to ordinary workflow children", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		mockPi.onCall({ exitCode: 1, stderr: "429 rate limit", matchArgIncludes: "Use nested model fallback" });
		mockPi.onCall({ output: "Nested fallback completed", matchArgIncludes: "Use nested model fallback" });
		const executor = makeExecutor([makeAgent("worker", {
			model: "openai/gpt-5-mini",
			fallbackModels: ["anthropic/claude-sonnet-4-5"],
		})]);
		...
		const result = await executor.executePublic(
			"workflow-child-model-fallback",
			{
				workflowScript: `return runs.run("ordinary", { agent: "worker", task: "Use nested model fallback", async: false, acceptance: false });`,
				async: false,
			},
			new AbortController().signal,
			undefined,

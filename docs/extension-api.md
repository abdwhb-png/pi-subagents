...
## Active subagent runs API (dormant compatibility addon)

The package still exports the process-global active-runs registry for rollback compatibility:

```ts
import {
  registerActiveSubagentRunSource,
  snapshotActiveSubagentRuns,
} from "pi-subagents/active-runs";
```

Sources share `Symbol.for("pi-subagents.active-runs.v1")`, so separately loaded package modules see the same state. A snapshot contains stable `{ id, sessionId, status? }` records for `queued`, `running`, and `paused` jobs owned by the exact Pi session.

The core pi-subagents extension no longer auto-registers its runtime as a source. Therefore `snapshotActiveSubagentRuns(sessionId)` returns no pi-subagents jobs unless a separately activated addon registers a source. The retained internal provider adapter is rollback code, not an active runtime dependency or public activation contract.

Keep this API separate from the background-work provider contract. Publishing pi-subagents' own runs as background work would make `subagent_wait` and headless auto-drain count those runs twice.

...
`);
	...
	it("creates, updates, and clears ordered model fallbacks", () => {
		const ctx = {
			cwd: tempDir,
			modelRegistry: {
				getAvailable: () => [
		...
		const created = handleCreate({
			config: {
				...
				description: "Worker with model fallback",
				scope: "project",
				fallbackModels: "openai/gpt-5-mini, anthropic/claude-sonnet-4, openai/gpt-5-mini",
			},
		}, ctx);
		assert.equal(created.isError, false);

		const filePath = path.join(tempDir, ".pi", "agents", "fallback-worker.md");
		let content = fs.readFileSync(filePath, "utf-8");
		assert.match(content, /^fallbackModels: openai\/gpt-5-mini, anthropic\/claude-sonnet-4$/m);

		const updated = handleUpdate({
			agent: "fallback-worker",
			config: { fallbackModels: [" anthropic/claude-sonnet-4 ", "openai/gpt-5-mini"] },
		}, ctx);
		assert.equal(updated.isError, false);
		content = fs.readFileSync(filePath, "utf-8");
		assert.match(content, /^fallbackModels: anthropic\/claude-sonnet-4, openai\/gpt-5-mini$/m);

		const cleared = handleUpdate({ agent: "fallback-worker", config: { fallbackModels: false } }, ctx);
		assert.equal(cleared.isError, false);
		assert.doesNotMatch(fs.readFileSync(filePath, "utf-8"), /^fallbackModels:/m);
	});

	it("rejects invalid model fallback management config", () => {
		const result = handleCreate({
			config: {
				name: "bad-fallback-worker",
				description: "Bad fallback models",
				scope: "project",
				fallbackModels: ["openai/gpt-5-mini", 42],
			},
		}, { cwd: tempDir, modelRegistry: { getAvailable: () => [] } });
		assert.equal(result.isError, true);
		assert.match(readText(result), /config\.fallbackModels must be a comma-separated string, string array, or false/);
	});

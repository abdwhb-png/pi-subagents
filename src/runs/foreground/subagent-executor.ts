...
import { buildModelFallbackWorkflowParams, isGeneratedModelFallbackWorkflow, isModelFallbackWorkflowAttempt } from "../shared/model-fallback-workflow.ts";
...
export function createSubagentExecutor(deps: ExecutorDeps): {
	...
	const execute = async (
		...
		if (!workflowChildPermitLaunch
			&& !isModelFallbackWorkflowAttempt(requestParams)
			&& typeof requestParams.agent === "string"
			&& typeof requestParams.task === "string") {
			const fallbackParentModel = parentModelOverride === null
				? undefined
				: parentModelOverride ?? normalizeParentModel(ctx.model);
			const fallbackCwd = resolveRequestedCwd(ctx.cwd, requestParams.cwd);
			const discovered = deps.discoverAgents(
				fallbackCwd,
				resolveExecutionAgentScope(requestParams.agentScope),
				fallbackParentModel?.provider,
			);
			const fallbackAgent = resolveAgentName(requestParams.agent, discovered.agents).agent;
			if (fallbackAgent?.fallbackModels?.length) {
				const explicitModel = typeof requestParams.model === "string" && requestParams.model.trim()
					? requestParams.model.trim()
					: undefined;
				const primaryModel = explicitModel
					?? fallbackAgent.model
					?? (fallbackParentModel ? `${fallbackParentModel.provider}/${fallbackParentModel.id}` : undefined);
				const fallbackWorkflow = buildModelFallbackWorkflowParams(requestParams, fallbackAgent.fallbackModels, primaryModel);
				if (fallbackWorkflow) requestParams = fallbackWorkflow.params as SubagentParamsLike;
			}
		}
		...
		if (requestParams.workflowScript !== undefined && normalizedAction === undefined) {
			...
			for (const warning of spawnBudgetValidation.warnings ?? []) {
				if (isGeneratedModelFallbackWorkflow(requestParams) && warning.kind === "dynamic-spawn-count") continue;

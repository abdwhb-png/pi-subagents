...
- Retain fork-local `fallbackModels` support as ordered fresh-session attempts. Retries are limited to provider/model startup failures before useful work, preserve foreground/background mode, consume normal workflow spawn capacity, and do not restore persistent model exclusions.

### Changed

- Stop auto-registering the custom active-runs bridge with the core extension. Keep the process-global API and internal provider adapter dormant for rollback without adding lifecycle behavior to normal runs.

...
- Use `output: false`, `defaultReads: false`, `defaultContext: false`, `acceptanceRole: false`, `machine: false`, or `fallbackModels: false` to clear an inherited value.
...
Simple-scalar list fields accept either a comma-separated form or a newline block list with one `- item` per line. This applies to `tools`, `excludeTools`, `defaultReads`, `skill`/`skills`, `skillPath`, `extensions`, `subagentOnlyExtensions`, and `fallbackModels`:
...
| `fallbackModels` | Fork compatibility extension: ordered backup models, each launched in a fresh child session only after a narrow provider/model startup failure. See [models.md](models.md#fresh-session-fallback-models-fork-compatibility-extension). |
